...
| `token_classes` | Compile a production whose alternatives are all single literals or tokens to one engine token set, named after it, that stands as one token at lookahead positions. The rule keeps its per-member alternates and its node. |
