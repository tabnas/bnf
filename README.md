...
| `tokenClasses` | Compile a production whose alternatives are all single literals or tokens (a keyword or operator class) to one engine token set, named after it, that stands as one token wherever a lookahead would otherwise enumerate its members. The rule keeps its per-member alternates and its node. For languages whose identifiers admit keywords. |
