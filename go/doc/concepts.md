...
| `liftLiteralTokens` | Gives a literal the name of the production that defines it, so `PL = "+"` becomes `#PL` rather than `#T1`. A production name holding whitespace, which an alternate's `s` would split into two token names, is not used; the literal keeps the name its text gives it. |
...
| `tokenClassNames` | Under `TokenClasses`, names the productions whose alternatives are all single literals or tokens; each becomes one engine token set. It skips a production named like an engine token, one whose name is empty or holds whitespace, one with a member that consumes nothing, and one whose set name the grammar already spells as a token, so every set is one token that consumes input and no set is ever a member of a set. The left-recursion substitution consumes a leading reference to one as that single token rather than inlining its alternatives, so the tree is unchanged and the fan-out is gone. |
